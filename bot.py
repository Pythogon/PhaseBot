...
class PhaseBot(commands.Bot):
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        await glo.STAR(message.strip("\n", " "), bot.get_channel(glo.STAR_CHANNEL_ID))
...
@commands.is_owner()
async def forcestar(ctx, channel: discord.TextChannel, message_id: int):
    gdpr_list = jsonRead("gdpr.json")
    try:
        if gdpr_list[str(ctx.author.id)] != 1: raise ValueError
    except:
        return await ctx.send(embed = glo.GDPR())
        try: await glo.STAR(await channel.fetch_message(message_id), bot.get_channel(glo.STAR_CHANNEL_ID)); await ctx.send(f"Message {message_id} starred.")
...
async def leaderboard(ctx):
    ...
    try:
        ...
        return await ctx.send(embed = glo.GDPR())
    read = dict(sorted(jsonRead("starcount.json").items(), key=operator.itemgetter(1),reverse=True))
    to_send = ""
...
async def eval_fn(ctx, *, cmd):
    ...
    insert_returns(body)
    env = {'bot': bot, 'discord': discord, 'commands': commands, 'ctx': ctx, '__import__': __import__, "date": date, "glo": glo}
